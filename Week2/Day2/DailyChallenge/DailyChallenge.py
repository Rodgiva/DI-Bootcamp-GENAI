...
class Pagination():
    def __init__(self, page_size:int = 10, items = None):
    ...
    def  get_visible_items(self)->list:
        return self.items[self.page_size*self._current_idx : self.page_size*(self._current_idx+1)]
    ...
    def last_page(self):
        self._current_idx = len(self.items)//self.page_size
        return self

    def next_page(self):
        if self._current_idx != len(self.items)//self.page_size:
            self._current_idx += 1
        return self
